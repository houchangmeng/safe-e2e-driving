...
def sample_reproduction_sensors(latent_model, device, state, action):
    state_ = {}
    state_['camera'] = torch.tensor(state['camera'], dtype=torch.uint8, device=device).float().div_(255.0)
    state_['lidar'] = torch.tensor(state['lidar'], dtype=torch.uint8, device=device).float().div_(255.0)
    state_['birdeye'] = torch.tensor(state['birdeye'], dtype=torch.uint8, device=device).float().div_(255.0)
    action = torch.tensor(action, dtype=torch.float32, device=device)

    feature_ = latent_model.get_features(state_)
    ...
    camera_mean_, state_std_ = latent_model.camera_decoder(z_)
    lidar_mean_, state_std_  = latent_model.lidar_decoder(z_)
    birdeye_mean_, state_std_= latent_model.birdeye_decoder(z_)

    camera_ = camera_mean_.cpu().detach().numpy()
    lidar_  = lidar_mean_.cpu().detach().numpy()
    birdeye_ = birdeye_mean_.cpu().detach().numpy()

    return camera_, lidar_, birdeye_
