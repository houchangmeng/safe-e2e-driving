...
class GlobalRoutePlanner(object):
    ...
    def _build_graph(self):
        ...
        for segment in self._topology:
            ...
            self._graph.add_edge(
                ...
                intersection=intersection, type='LaneFollow')
            ...
            if road_id in self._road_id_to_edge \
                ...
                if path:
                    ...
                    self._graph.add_edge(
                        ...
                        intersection=end_wp.is_junction, type='LaneFollow')
                ...
                if not segment['entry'].is_junction:
                    ...
                    if waypoint.right_lane_marking and waypoint.right_lane_marking.lane_change & carla.LaneChange.Right and not right_found:
                        ...
                        if next_waypoint is not None \
                            ...
                            next_road_option = 'ChangeLaneRight'
                    ...
                    if waypoint.left_lane_marking and waypoint.left_lane_marking.lane_change & carla.LaneChange.Left and not left_found:
                        ...
                        if next_waypoint is not None \
                                and next_waypoint.lane_type == carla.LaneType.Driving \
                                and waypoint.road_id == next_waypoint.road_id:
                            next_road_option = 'ChangeLaneLeft'
                            next_segment = self._localize(next_waypoint.transform.location)
                            if next_segment is not None:
                                self._graph.add_edge(
                                    self._id_map[segment['entryxyz']], next_segment[0], entry_waypoint=waypoint,
    ...
    def _successive_last_intersection_edge(self, index, route):
        ...
        for node1, node2 in [(route[i], route[i+1]) for i in range(index, len(route)-1)]:
            ...
            if candidate_edge['type'] == 'LaneFollow' and candidate_edge['intersection']:
                last_intersection_edge = candidate_edge
                last_node = node2
            else:
                break
    ...
    def _turn_decision(self, index, route, threshold=math.radians(35)):
        ...
        if index > 0:
            if self._previous_decision != 'Void' \
                    and self._intersection_end_node > 0 \
                    and self._intersection_end_node != previous_node \
                    and next_edge['type'] == 'LaneFollow' \
                    and next_edge['intersection']:
                decision = self._previous_decision
            else:
                self._intersection_end_node = -1
                current_edge = self._graph.edges[previous_node, current_node]
                calculate_turn = current_edge['type'] == 'LaneFollow' and not current_edge[
                    'intersection'] and next_edge['type'] == 'LaneFollow' and next_edge['intersection']
                if calculate_turn:
                    ...
                    for neighbor in self._graph.successors(current_node):
                        ...
                        if select_edge['type'] == 'LaneFollow':
...
def main():
    ...
    ego_bp = random.choice(bp.filter('*vehicle*'))
    # set ego attribute
    ego_bp.set_attribute('color','0,0,0')
    # spawn actor
    transforms = world.get_map().get_spawn_points()
    ego = world.spawn_actor(ego_bp,transforms[0])
    ...
    spectator.set_transform(
        carla.Transform(transform.location + 
            carla.Location(x=-0.1,y=0.0,z=50),
            carla.Rotation(roll=0,yaw=180,pitch=-90)))
    
    global_planner = GlobalRoutePlanner(world.get_map() ,sampling_resolution=2.0)
    start_location = transforms[0].location
    end_location = transforms[1].location
    path = global_planner.trace_route(start_location,end_location)
