...
import os
import glob
from gym_carla.envs.misc import display_to_rgb
...
class MapImage(object):
    ...
    def __init__(self, carla_world, carla_map, pixels_per_meter):
        ...
        dirname = os.path.join("cache", "map_image")
    ...
    def draw_road_map(
        ...
        def get_lane_markings(
            ...
            if lane_marking_type == carla.LaneMarkingType.Broken or (
                ...
                marking_2 = [world_to_pixel(lateral_shift(
                    w.transform, sign * (w.lane_width * 0.5 + margin * 2))) for w in waypoints]
                if lane_marking_type == carla.LaneMarkingType.SolidBroken:
                    return [(carla.LaneMarkingType.Broken,
                             lane_marking_color,
                             marking_1),
                    ...
                    return [(carla.LaneMarkingType.Solid,
                             lane_marking_color,
                             marking_1),
            ...
            return [(carla.LaneMarkingType.NONE,
        ...
        def draw_traffic_signs(
            ...
            left_vector = carla.Location(-forward_vector.y,
                                         forward_vector.x,
            ...
            pygame.draw.lines(surface, color, True, line_pixel, 3)
        ...
        def draw_topology(carla_topology, index):
            ...
            for waypoint in topology:
                ...
                # ==============annotation to simplify roadmap========== #
...
class BirdeyeRender(object):
    def __init__(self, world, params, display_man, display_pos,global_view=False):
        self.params = params
        self.server_fps = 0.0
        ...
        self.disp_size = self.display_man.get_display_size()  # width hight
        ...
        self.max_past_step = self.params['max_past_step']
        ...
        self.original_surface_size = min(self.disp_size[0], self.disp_size[1])
        ...
        self.local_surface = pygame.Surface(
            (self.disp_size[0], self.disp_size[0])).convert()

        self.data = None

        weak_self = weakref.ref(self)
        self.world.on_tick(
            lambda timestamp: BirdeyeRender.on_world_tick(
                weak_self, timestamp))
    ...
    def reset_surface(self):
        ...
        self.global_surface = pygame.Surface(
            (self.disp_size[0], self.disp_size[0])).convert()
        self.local_surface = pygame.Surface(
            (self.disp_size[0], self.disp_size[0])).convert()
        

    def set_hero(self, hero_actor):
    ...
    def tick(self, clock):
        ...
        if self.hero_actor.is_alive or self.hero_actor is not None:
    ...
    def render_waypoints(self, surface, waypoints, world_to_pixel, width=20):
        if self.red_light:
            # purple
            color = pygame.Color(
                math.floor(
        ...
        while len(waypoints) < 40 :
            # ==============annotation to continue render roadmap========== #
            waypoints.append(waypoints[-1].next(3)[0])
        for w in waypoints:
            p = [w.transform.location.x, w.transform.location.y, w.transform.rotation.yaw]
            corners.append(carla.Location(x=p[0], y=p[1]))
        corners = [world_to_pixel(p) for p in corners]
        pygame.draw.lines(surface, color, False, corners, width=width) # waypoints width
        # for c in corners:
        #     pygame.draw.circle(surface, color, c, width / 2)
    ...
    def render(self, render_types= ['roadmap', 'actors','waypoints']):
        # clock tick
        ...
        if self.hero_actor.is_alive:
            angle = self.hero_transform.rotation.yaw + 90.0
        else:
            angle = 0.0

        # angle = 0.0 if not self.hero_actor.is_alive or  else self.hero_transform.rotation.yaw + 90.0

        if self.global_view:
            Util.blits(self.result_surface, surfaces)
            self.global_surface = pygame.transform.smoothscale(
                self.result_surface, self.disp_size)

            offset = self.display_man.get_display_offset(self.display_pos)
            self.display_man.display.blit(self.global_surface, offset)
            ...
            rotated_result_surface = pygame.transform.rotozoom(
                self.hero_surface, angle, 1.0).convert()

            center = (self.disp_size[0] / 2, self.disp_size[1] / 2)
            rotation_pivot = rotated_result_surface.get_rect(center=center)

            self.local_surface.blit(rotated_result_surface, rotation_pivot)
            offset = self.display_man.get_display_offset(self.display_pos)
            self.display_man.display.blit(self.local_surface, offset)

            birdeye = pygame.surfarray.array3d(self.local_surface)
            self.data = display_to_rgb(birdeye, self.disp_size).astype('uint8')
...
def run_simulation(args, client):
    """
    Initialized, Starts and runs all the needed modules for BEV Rendering Mode.
    """
    ...
    try:
        ...
        camera4 = CameraManager(Cameras['dvs'], vehicle, carla.Transform(carla.Location(x=0, z=2.4), carla.Rotation(yaw=0)), 
                      display_manager,  display_pos=[1, 1])
        sensor_list.append(camera4.sensor)
        camera5 = CameraManager(Cameras['optical_flow'], vehicle,carla.Transform(carla.Location(x=0, z=2.4), carla.Rotation(yaw=0)), 
                      display_manager,  display_pos=[1, 2])
        sensor_list.append(camera5.sensor)

        display_size = display_manager.get_display_size()[0]
        obs_range = 32
        pixels_per_meter = display_size / obs_range
        d_behind = 12
        pixels_ahead_vehicle = (obs_range / 2 - d_behind) * pixels_per_meter
        birdeye_params = {
            'screen_size': display_manager.get_display_size(),
            'pixels_per_meter': pixels_per_meter,  # 定义俯视图视野大小
            'pixels_ahead_vehicle': pixels_ahead_vehicle,
            'global':True
        }
        birdeye_render = BirdeyeRender(world, birdeye_params, display_manager,
                                   display_pos=[1, 0])
        birdeye_render.set_hero(vehicle)
...
if __name__ == '__main__':
    from sensors_manger import DisplayManager,CameraManager,Cameras,CollisionSensor,LaneInvasionSensor,GnssSensor,IMUSensor,RadarSensor,CustomTimer
    import argparse
   
    from misc import display_to_rgb
    main()
